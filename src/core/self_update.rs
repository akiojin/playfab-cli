...
pub fn maybe_self_update() -> Option<JoinHandle<()>> {
    if std::env::var("PLAYFAB_CLI_NO_AUTO_UPDATE").ok().as_deref() == Some("1") {
        tracing::debug!("self-update skipped: PLAYFAB_CLI_NO_AUTO_UPDATE=1");
        return None;
    }
...
/// Print a one-time warning if `~/.cargo/bin/playfab-cli` exists, which could
/// shadow the managed binary.
pub fn warn_cargo_conflict() {
    if let Some(home) = dirs::home_dir() {
        let cargo_bin = if cfg!(target_os = "windows") {
            home.join(".cargo/bin/playfab-cli.exe")
        } else {
            home.join(".cargo/bin/playfab-cli")
        };
        if cargo_bin.exists() {
            eprintln!(
                "warning: {} exists and may shadow the managed binary. \
                 Consider running `cargo uninstall playfab-cli`.",
                cargo_bin.display()
            );
        }
    }
...
fn last_check_path() -> anyhow::Result<PathBuf> {
    Ok(install_dir_for(ManagedBinary::PlayFabCli)?.join("LAST_UPDATE_CHECK"))
}
...
fn run_update(marker: &Path) -> anyhow::Result<()> {
    let latest = fetch_latest_release(ManagedBinary::PlayFabCli);

    // Always touch the marker, even on failure, to prevent hammering the API.
    touch(marker);

    let latest = latest?;
    let local = read_local_version_for(ManagedBinary::PlayFabCli);
    ...
    let dest = binary_path_for(ManagedBinary::PlayFabCli)?;
    download_latest_binary(ManagedBinary::PlayFabCli, &latest, &dest)?;
