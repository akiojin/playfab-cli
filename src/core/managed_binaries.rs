...
impl ManagedBinary {
    fn spec(self) -> ManagedBinarySpec {
        match self {
            Self::PlayFabCli => ManagedBinarySpec {
                key: "playfab-cli",
                install_subdir: "playfab-cli",
                manifest_name: "playfab-cli-manifest.json",
                executable_name: if cfg!(target_os = "windows") {
                    "playfab-cli.exe"
                } else {
                    "playfab-cli"
                },
                repos: &["akiojin/playfab-cli"],
            },
        }
    }
}
...
pub fn executable_name() -> &'static str {
    ManagedBinary::PlayFabCli.spec().executable_name
}

pub fn tools_root() -> Result<PathBuf> {
    env::var("PLAYFAB_CLI_TOOLS_ROOT")
        .ok()
        .map(|root| PathBuf::from(root.trim()))
        .or_else(|| dirs::home_dir().map(|home| home.join(".playfab/tools")))
        .ok_or_else(|| anyhow!("Unable to resolve tools root"))
}

pub fn cli_install_latest(force_download: bool) -> Result<Value> {
    Ok(install_latest_for(ManagedBinary::PlayFabCli, force_download)?.to_json())
}

pub fn cli_doctor() -> Result<Value> {
    Ok(doctor_for(ManagedBinary::PlayFabCli)?.to_json())
}

pub fn cli_status() -> Result<ManagedBinaryStatus> {
    inspect_for(ManagedBinary::PlayFabCli)
}

pub fn ensure_latest_cli_for_daemon() -> Result<ManagedBinaryStatus> {
    ensure_latest_for(ManagedBinary::PlayFabCli, false, true)
}
...
pub fn binary_path() -> Result<PathBuf> {
    binary_path_for(ManagedBinary::PlayFabCli)
}

#[cfg(test)]
pub fn version_path() -> Result<PathBuf> {
    version_path_for(ManagedBinary::PlayFabCli)
}

#[cfg(test)]
pub fn read_local_version() -> Option<String> {
    read_local_version_for(ManagedBinary::PlayFabCli)
}
...
pub fn ensure_local(force_download: bool) -> Result<PathBuf> {
    ensure_local_for(ManagedBinary::PlayFabCli, force_download)
}
...
fn write_local_version(version: &str) -> Result<()> {
    write_local_version_for(ManagedBinary::PlayFabCli, version)
}
...
fn should_skip_remote_checks_for_tests() -> bool {
    cfg!(test)
        && matches!(
            std::env::var("PLAYFAB_CLI_TEST_SKIP_MANAGED_UPDATE")
                .ok()
                .as_deref(),
...
mod tests {
    ...
    fn unique_temp_path(label: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("playfab-cli-managed-{label}-{nanos}"))
    }
    ...
    #[test]
    fn tools_root_prefers_playfab_cli_tools_root_env() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(|poison| poison.into_inner());
        let root = unique_temp_path("tools-root");
        let root_with_spaces = format!("  {}  ", root.display());
        let _env = EnvVarGuard::set("PLAYFAB_CLI_TOOLS_ROOT", &root_with_spaces);
    ...
    #[test]
    fn playfab_cli_status_uses_managed_install_layout() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(|poison| poison.into_inner());
        ...
        let binary =
            binary_path_for(ManagedBinary::PlayFabCli).expect("binary path should resolve");
        if let Some(parent) = binary.parent() {
            fs::create_dir_all(parent).expect("binary parent directory should be created");
        }
        fs::write(&binary, b"managed-playfab-cli").expect("binary fixture should be writable");
        write_local_version_for(ManagedBinary::PlayFabCli, "0.3.0")
            .expect("playfab-cli version should be writable");
        ...
        assert_eq!(json["key"], "playfab-cli");
        assert_eq!(
    ...
    fn ensure_latest_for_can_skip_remote_checks_in_tests() {
        ...
        let _skip = EnvVarGuard::set("PLAYFAB_CLI_TEST_SKIP_MANAGED_UPDATE", "1");

        let binary = binary_path_for(ManagedBinary::PlayFabCli)
            .expect("playfab-cli binary path should resolve");
        if let Some(parent) = binary.parent() {
            fs::create_dir_all(parent).expect("binary parent directory should be created");
        }
        fs::write(&binary, b"managed-playfab-cli").expect("binary fixture should be writable");

        let status = ensure_latest_for(ManagedBinary::PlayFabCli, false, true)
